...
async function openChat(element) {
    ...
    // Reveal the continue-chat-button on this specific post (stays visible from now on)
    ...
    // ── Highlight — always the post card ─────────────────────────
...
$(window).on('load', function () {
    ...
    $('.chat-minimize').on('click', function (e) {
        ...
        $history.slideToggle(300, 'swing', function () {
            if (chatUIState.currentChatId) {
                postChatState.set(
                    chatUIState.currentChatId,
                    $history.is(':visible') ? 'open' : 'minimized'
                );
            }
        });
